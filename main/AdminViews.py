...
def Import_Excel_pandas(request):
    if request.method =="POST":
        ...
        for sheet_name in workbook.sheetnames:
            ...
            print(f"Работа с листом: {sheet.title}")
            # Взять 1 столбец для добавления в категории и для последующих действий
            ...
            if message_from_category == "Import success!":
                ...
                if message_from_products == "Import success!":
                    ...
                    for row in sheet.iter_rows(min_col = 10,max_col=27, values_only=True):
                        row_data = list(row)
                    ...
                    for row in sheet.iter_rows(min_col = 29,max_col=47, values_only=True):
                        row_data = list(row)
                    ...
                    for row in sheet.iter_rows(min_col = 49,max_col=76, values_only=True):
                        row_data = list(row)
